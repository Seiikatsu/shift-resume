  ...
  return (
    <Button variant="ghost" className="justify-start gap-2" asChild>
      <Link to={href}>
        <Icon size={16} />
        {sidebarOpen ? (
          <Typography tag="span" messageId={messageId}>
            {messageId}
          </Typography>
        ) : null}
      </Link>
        ...
        <div className="flex h-full flex-col">
          ...
          <ScrollArea className="flex-1 px-4">
            <nav className="flex flex-col space-y-2">
              <NavigationItem
                href="/resumes"
                messageId="common:navigation.item.resumes"
                Icon={IconUser}
          ...
          <div className="mt-auto border-t border-secondary-foreground/10 px-4 py-4">
            <nav className="flex flex-col space-y-2">
              <NavigationItem
                href="/profile"
                messageId="common:navigation.item.profile"
                Icon={IconUser}
                sidebarOpen={isSidebarOpen}
              />
              <NavigationItem
                href="/settings"
                messageId="common:navigation.item.settings"
                Icon={IconSettings}
                sidebarOpen={isSidebarOpen}
              />
            </nav>
