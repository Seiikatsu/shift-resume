...
const Resume: FC<{ id: string; title: ReactNode; lastEdit: string; template?: boolean }> = (resume) => {
  return (
    <Link to={`/resume/${resume.id}`}>
      <div
        className="cursor-pointer border bg-neutral-900 border-neutral-700 h-[600px] w-96 relative transition-transform hover:scale-105">
        {resume.template ? (
...
export default function Resumes() {
  ...
  return (
    <div className="flex gap-8 flex-wrap">
      <Resume id="create" title={<Typography tag="h3" variant="paragraph" messageId="resumes.create-resume"/>}
              lastEdit="" template/>
      {resumes.map((resume) => (
        <Resume key={resume.id}
                {...resume}
                title={
                  <h3 className="text-neutral-100">{resume.title}</h3>
                }
        />
      ))}
    </div>
