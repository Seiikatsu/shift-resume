...
import type { Section } from '~/components/sectionBuilder/preview/types';

type SectionPreview = Omit<Section, 'fields'>;

export const SectionPreview: FC<SectionPreview> = ({ id: _id, label }) => {
  const { fields } = useContext(SectionPreviewContext);
  return (
    <div className="w-full border rounded-md border-neutral-200 shadow-md p-4">
      <h2 className="mb-4 text-2xl">{label}</h2>
      <DndProvider backend={HTML5Backend}>
        <div className="flex flex-col gap-2">
          {fields.map((field) => {
            ...
            return <DraggableContentField key={field.id} {...field} />;
          })}
        </div>
      </DndProvider>
    </div>
